...
Google Cloud - Cloud Provider Registration
===========================

##   Pre-requisite Steps

Before creating a Cloud Provider, you will need to create a Service Account Key from the **API Manager** in the Google Cloud Console.  Save the generated JSON file.


##   Register a Cloud Provider for Google Cloud

You can now register a Cloud Provider for Google Cloud by navigating to **Cloud Providers** and then clicking on the **+New** button to select **Google Cloud**. The required fields are:
-   **Name** - a user can create multiple Cloud Providers with the same Google Cloud credentials but with different names and entitlements to control infrastructure provisioning by team or department
-   **Client Email** - this is the email used with this Google Cloud account.
-   **JSON Key File** - this is the generated JSON file that is associated with the service account created
